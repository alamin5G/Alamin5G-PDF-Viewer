...
import com.alamin5g.pdf.viewer.util.PageSizeCalculator;
...
class PdfFile {
    ...
    PdfFile(PdfRenderer pdfRenderer, ParcelFileDescriptor fileDescriptor, FitPolicy pageFitPolicy, Size viewSize, int[] originalUserPages,
    ...
    private void setup(Size viewSize) {
        ...
        for (int i = 0; i < pagesCount; i++) {
            PdfRenderer.Page page = pdfRenderer.openPage(documentPage(i));
            Size pageSize = new Size(page.getWidth(), page.getHeight());
            page.close();
    ...
    public boolean openPage(int pageIndex) throws PageRenderingException {
        ...
        synchronized (lock) {
            if (openedPages.indexOfKey(docPage) < 0) {
                try {
                    // 16KB Compatible: Use Android's native PdfRenderer
                    PdfRenderer.Page page = pdfRenderer.openPage(docPage);
                    page.close(); // Just test if we can open it
    ...
    public Object getMetaData() {
        // 16KB Compatible: Android's PdfRenderer doesn't support metadata
        return null;
    }

    public List<Object> getBookmarks() {
        // 16KB Compatible: Android's PdfRenderer doesn't support bookmarks
        return new ArrayList<>();
    }

    public List<Object> getPageLinks(int pageIndex) {
        // 16KB Compatible: Android's PdfRenderer doesn't support links
        return new ArrayList<>();
    }

    public RectF mapRectToDevice(int pageIndex, int startX, int startY, int sizeX, int sizeY,
                                 RectF rect) {
        // 16KB Compatible: Android's PdfRenderer doesn't support coordinate mapping
        return rect;
    }

    public void dispose() {
        // 16KB Compatible: Clean up Android's PdfRenderer
        if (pdfRenderer != null) {
            pdfRenderer.close();
        }
